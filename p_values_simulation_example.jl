...
function estimate_risks(β)
    ...
    B = 500
    rmse_irr = Array{Float64}(undef, B)
    rmse_reg = Array{Float64}(undef, B)
    ...
    for j in eachindex(π0s)
        for i in eachindex(ns)
            for b = 1:B
                ...
                H_irr, _ = histogram_irregular(x; support=(0.0, 1.0), a=1.0, grid="quantile")
                H_reg, _ = histogram_regular(x; support=(0.0, 1.0), a=k->0.5*k)
