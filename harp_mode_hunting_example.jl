...
function generate_harp_mode_plot()
    ...
    t = LinRange(-3, 80, 10000)
    p1 = plot(xlabel="x", ylabel="Density", title="BIC")
    plot!(p1, t, pdf.(d, t), lwd=3.0, color="blue", label="")
    ...
    scatter!(p1, reg_modes[[1,2,3,4,10]], zeros(4), color="red", label="", linecolor="red")
    scatter!(p1, reg_modes[setdiff(1:end, [1,2,3,4,10])], zeros(10), color="black", label="")

    p2 = plot(xlabel="x", ylabel="Density", title="Random irregular histogram")
    plot!(p2, t, pdf.(d, t), lwd=3.0, color="blue", label="")
